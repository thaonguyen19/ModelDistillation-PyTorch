...
def soft_target_loss(soft_targets, soft_outputs):
    soft_targets = soft_targets.cuda(async=True)
    soft_outputs = soft_outputs.cuda(async=True)
    soft_target_var = torch.autograd.Variable(soft_targets, requires_grad=False)
    soft_output_var = torch.autograd.Variable(soft_outputs, requires_grad=False) #REQUIRES_GRAD=??????????
    loss = torch.nn.BCELoss().cuda()
    return loss(soft_output_var, soft_target_var)
...
def get_datasets(train_fnames, val_fnames,
    ...
    ######################CHECK 0-1 LABELS###########################
    #positive_data = len([data for data in train_imgs if data[1]==1])
    #negative_data = len(train_imgs) - positive_data
    #print "TESTING: number of positive data: %d   number of negative data: %d" % (positive_data, negative_data)
    #################################################################
